...
There is no test suite; the smoke test runs against the real feeds (they are
always live). For a quick pipeline check use a permissive config
(`postcodes = []`, `keywords = []`, `regions = []`) and watch `[debug] cycle:`
lines plus `[dry-run]` notifications. Docker: `docker build -t p2000-chirp .`
then run with `-e DRY_RUN=true` and the volume mounts from
`docker-compose.yaml`.
...
| `src/filter.ts` | Area (OR across axes) + discipline (AND) matching, discipline inference |
| `src/state.ts` | Persistent seen-store: message text → ts, trailing sequence → ts |
| `src/tz.ts` | Europe/Amsterdam wall-clock → UTC parsing (DST-correct) |
| `src/telegram.ts` | sendMessage call |
...
- **Berkel-Enschot RSS** (`p2000.brandweer-berkel-enschot.nl/homeassistant/rss.asp`):
  ...
  - Politie items usually have empty `RegName`/`Dienst` fields; discipline
    is inferred from the message prefix (`A1/A2`, `B1/B2`, `P1/P2`).
...
- **Dedupe design**: identity is message TEXT + trailing sequence number
  within a time window, NOT timestamps — mirrors stamp times differently and
  radio frames arrive corrupted (same `bon 138437`, garbled text). The window
  (`dedupe_window_seconds`) drives suppression. Do not "fix" dedupe to use
  timestamps; it was tried and it double-notifies.
